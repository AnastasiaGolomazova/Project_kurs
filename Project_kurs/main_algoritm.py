...
class Analysiator():
    ...
    def __init__(self, json, vector_path, list_object):
        self.json_file = json
        ...
        for index in range(len(vector_path)-1):
            ...
            if self.contains(['*', "="]): # ситуация, если мы объявляем указатель and ситуация, если мы объявляем указатель и присваиваем значение
                ...
                if self.checkNULL(self.lines) == False: # специальная проверка для методов, которые создают новый список узлов
                    ...
                    self.log(f'<{memory[new_num]},{new_num}>', f'+<{environment[head]},{head}>')
                    new_num+=1
            ...
            ######         
                ...
                ## откуда old?
                self.log(f'<{old_ptr},{memory[old_ptr]}>', f'+<{environment[left]},{left}>')
                new_num += 1
                ...
                if memory[n] == None:
                    ...
                    self.log(f'<{n},{memory[n]}>', f'+<{environment[left]},{left}>')
